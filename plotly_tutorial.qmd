...
::: notes
Before looking at bivariate plots lets look at how we can use colors in our plots.
:::
...
::: notes
Here we are looking at a bar plot we made at the beginning which tracks sleep hours for three days of the week. We have added a color argument and set it to day which will color each bar based the day. Just like that we can add colors to our plots. Plotly automatically assigns a unique color to every category in that column, or generates a gradient scale if the column is numerical. But we can customize color scales however we want.
:::
...
::: notes
We can also make the color dictionary separately. Here i have created the same dictionary and called it my colors.
:::
...
::: notes
Here I have used this dictionary inside the box plot. So far we have looked at colors to map discrete variables. What if we are dealing with numbers instead of categories?
:::
...
::: notes
Now lets look at bivariate visualizations. Bivariate visualizations are visualizations on datasets that have two variables. Lets focus on three most common plots types for bivariate data visualizations. The first is a scatter plot.
:::
...
:::notes
Let's start with the classic scatter plot. This plot relies on a Cartesian system: we have an X-axis representing your first variable and a Y-axis for the second. Wherever those two values meet for a single data point, we place a marker. For instance, here we have a scatter plot using the scatter function that looks at sepal and petal lengths of iris flowers. We can see that as sepal length increases so does the respective petal lengths.
:::
...
::: notes
Plotly lets you customize this even further. We can also map a categorical column to the 'symbol' argument. This changes the dots into squares or diamonds depending on the category. I have set it to the species."
:::
...
::: notes
We can overlay our plot with a trendline. Trendlines could be linear or non-linear in nature. Here we have used ols or ordinary least squares trendline which is a linear regression line. Other types of trendlines could be use as well such as lowess or moving averages.
:::
...
::: notes
The second type of bivariate plot is a line plot. These are your go-to for tracking changes over a period. Line plots are simple to make using the line function in plotly. Here we are looking microsoft stocks from the stocks dataset. The x axis is set to date and y axis is set to the stock value over time.
:::
