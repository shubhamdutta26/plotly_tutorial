...
::: notes
Now, what if you want to check relationships between many variables at once? That is where correlation plots come in. For correlation plots we need a statistical parameter called the Correlation Coefficient. It gives us a number between negative one and positive one. Lets see how to make a correlation plot.
:::
...
::: notes
Now let's visualize it with a heatmap using the function called imshow. We pass our new correlation matrix into it. And we get a correlation matrix plot.
:::
...
::: notes
We can Set text_auto to True to display the actual numbers in the boxes.
:::
...
::: notes
Additionally we can fixed the min-max values to -1 and 1 using the zmin and zmax arguments, ensuring proper color representation of correlation strength.
:::

# Customizing plots

::: notes
Next we’re going to level up our visualizations by mastering the art of interaction. We are diving into how to customize those pop-up hover details and fine-tune our chart legends to tell a better story.
:::
...
::: notes
Lets take a look at this scatter plot. Hovering our mouse on any of these points it displays the respective sepal and petal lengths. However we dont know which species the points represent.
:::
...
::: notes
We can style the hover labels using the hoverlabel argument. This takes a dictionary where we can tweak things like the font family or background color. There is a more complex method called hovertemplate for full HTML control which you will be able to find in the online documentation of plotly.
:::
...
::: notes
To position it, we provide the legend dictionary with x and y coordinates. These aren't pixels; they are normalized coordinates between 0 and 1. We can also set a background color. As always, the documentation has a full list of properties that you can look at.
:::
...
:::{.detective data-detective-search="subplot_titles"}
```{python}
fig = make_subplots(
    rows=3, cols=1,
    subplot_titles=\
...
::: notes
We generate two plots: one is a scatter plot for Boston and the other is a line plot for Houston.
:::
...
::: notes
Finally, we fuse them together by creating a new combined figure that unpacks the data from both the scatter and line charts using the star operator. If you dont know what a star operator is i highly recommend you to watch my video on Iterators and comprehensions. The link is in the decription below.
:::
...
::: notes
We don't always have to build everything at once. Sometimes we might have an existing chart and we want to add that new metric to our plot. Lets add the seattle weather to our figure. First, we make a new line chart with the seattle weather.
:::
